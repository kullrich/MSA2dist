...
KAKS::KAKS() {
  ...
  "Model"}; //25 outputting items
	...
	//int i;
  std::size_t i;
	for (i=0; i<sizeof(items)/sizeof(string); i++) {
    titleInfo.push_back(items[i]);
  }
    ...
    Rcpp::StringVector cdsstrnames = cdsstr.attr("names");
      ...
      for (j=i+1; j<n; j++ ) {
        ...
        if (checkValid(name, name1, name2, comp1, comp2, str.substr(0, str.length()/2), str.substr(str.length()/2, str.length()/2))) {
          ...
          if (!calculateKaKs()) {	//calculate Ka&Ks using selected method(s)
            throw 1;
          }
				  if (VERBOSE) {
            Rcpp::Rcout<<"[OK]"<<endl;
...
bool KAKS::parseParameter(string sgc, string method, bool verbose) {
    ...
    if (temp=="NONE") {
      none=true;
		...
		else if (temp=="GNG") { //added by zhangyubin
		  gng86=true;
		}
		else if (temp=="LWL") {
		  lwl85=true;

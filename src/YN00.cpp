...
int YN00::DistanceF84(double n, double P, double Q, double pi4[], double &k_HKY, double &t, double &SEt) {
	...
	double Qsmall=min2(1e-10,0.1/n), maxkappa=2,maxt=99;
	k_HKY=-1;
	Y=pi4[0]+pi4[1];
	R=pi4[2]+pi4[3];
	tc=pi4[0]*pi4[1];
	...
	if (!failF84) {
		if ((GAMMA==4)||(GAMMA==20)) {
		    a=.5*GAMMA*(pow(a,-1.0/GAMMA)-1);
		    b=.5*GAMMA*(pow(b,-1.0/GAMMA)-1);
		}
		else {
			a=-.5*log(a); b=-.5*log(b);
		}
...
int YN00::DistanceYN00(const string seq1, const string seq2, double &dS,double &dN, double &SEKs, double &SEKa) {
	...
	for (ir=0; ir<(iteration?nround:1); ir++) {   /* iteration */
		...
		YNCountDiffs(seq1, seq2, Sdts, Sdtv, Ndts, Ndtv, PMatrix);
		Sd=Sdts+Sdtv;
		Nd=Ndts+Ndtv;
		//synonymous
...
int YN00::YNCountDiffs(const string seq1, const string seq2, double &Sdts,double &Sdtv,double &Ndts, double &Ndtv,double PMatrix[]) {
